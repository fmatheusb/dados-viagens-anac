...
caminho_pasta = r"C:\Users\fmath\Documents\dados-viagens-anac\dados-viagens-anac\data\empresas\arquivos_brutos"

# Caminho final para o csv tratado
caminho_final = r"C:\Users\fmath\Documents"

# Pasta temporária para descompactar os arquivos
pasta_temporaria = r"C:\Users\fmath\Documents\dados-viagens-anac\dados-viagens-anac\data\empresas\temp"
    ...
    if nome_arquivo.endswith('.CSV') or nome_arquivo.endswith('.csv'):
        ...
        try:
            ...
            df_temp = pd.read_csv(io.StringIO(conteudo_str), encoding='utf-8', sep=None, engine='python'
                                    , usecols=["Razão Social", "ICAO", "IATA", "Atividades Aéreas", "ID Empresa Aerea"])
            
            #Renomeando colunas
            df_temp.rename(columns={'Razão Social': 'razao_social',
                                    'ICAO': 'icao',
                                    'IATA': 'iata',
                                    'Atividades Aéreas': 'tipo',
                                    'ID Empresa Aerea': 'id'}, inplace=True)
            ...
            try:
                ...
                df_temp = pd.read_csv(io.StringIO(conteudo_str), encoding='utf-8', sep=None, engine='python'
                                      , usecols=["Razão Social", "ICAO", "Atividades Aéreas", "id_empresa_aerea"])

                # Renomeia as colunas para padronizar
                df_temp.rename(columns={'Razão Social': 'razao_social',
                                        'ICAO': 'icao',
                                        'Atividades Aéreas': 'tipo',
                                        'id_empresa_aerea': 'id'}, inplace=True)

                # Seleciona as colunas corretas para igualar os dois dataframes
                df_temp = df_temp.loc[:, ['razao_social', 'icao', 'tipo', 'id']]

                # Como a coluna IATA não tem nesse padrão 2, o arquivo segue com os valores nulos
                df_temp['iata'] = None

                #Ajustando a ordem das colunas
                df_temp = df_temp[['razao_social', 'icao', 'iata', 'tipo', 'id']]
...
# Remover duplicadas
df_final.drop_duplicates('id',inplace=True)

# Envia o dataframe tratado para uma pasta
df_final.to_csv(f"{caminho_final}\\base_empresas_tratada.csv", index=False)
